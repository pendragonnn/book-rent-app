  ...
  <div class="py-10" style="background-color: #F9F3EF">
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="bg-white border border-[#d2c1b6] shadow-md rounded-xl p-6 mb-6">
        ...
        <form action="{{ route('admin.book-receipts.update', $receipt->id) }}" method="POST"
            ...
            <div class="space-y-2">
              ...
              <select name="user_id" id="user_id" required
                ...
                @foreach ($users as $user)
                  <option value="{{ $user->id }}" {{ $receipt->user_id == $user->id ? 'selected' : '' }}>
            ...
            <div class="space-y-2">
              ...
              <select name="status" id="status" required
                ...
                @foreach ($statuses as $status)
                  <option value="{{ $status }}" {{ $receipt->status == $status ? 'selected' : '' }}>
                    {{ ucfirst($status) }}
                  </option>
                @endforeach
              </select>
              ...
              @foreach($receipt->loans as $index => $loan)
                <tr>
                  ...
                  <td class="px-4 py-3">
                    ...
                    @endphp
                    <span
                      class="text-white px-2 py-1 rounded-full text-xs font-semibold flex text-center items-center justify-center {{ $statusColor }}">
                      {{ ucwords(str_replace('_', ' ', $loan->status)) }}
                    </span>
                  </td>
                  <td>
                    @if ($loan->status === 'admin_validation')
                      <form action=" {{ route('admin.book-loans.cancelLoan', $loan->id) }}" method="POST"
