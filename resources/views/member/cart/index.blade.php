...
<table class="w-full border-collapse border border-[#d2c1b6]">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="border p-2">{{ \App\Models\BookItem::find($item['book_item_id'])->book->title }}</td>
            <td class="border p-2">{{ \Carbon\Carbon::parse($item['loan_date'])->format('d M Y') }}</td>
            <td class="border p-2">{{ \Carbon\Carbon::parse($item['due_date'])->format('d M Y') }}</td>
            <td class="border p-2 text-center">{{ $item['days'] }} hari</td>
            <td class="border p-2">Rp{{ number_format($item['total_price'], 0, ',', '.') }}</td>
    ...
    <div class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6">
        ...
        <form action="{{ route('member.receipts.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-2 mb-4">
                ...
                <label class="border p-3 rounded-lg cursor-pointer flex items-center gap-2">
                    <input type="radio" name="payment_method" value="cod" required> COD
                </label>
            </div>
