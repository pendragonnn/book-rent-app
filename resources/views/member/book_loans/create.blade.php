    ...
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-2xl shadow-md border border-[#d2c1b6]">
      <div class="flex flex-col md:flex-row gap-6">
        ...
        <div>
          ...
          <p class="text-[#555]">💰 Harga Sewa: <span class="font-semibold text-[#1B3C53]">Rp{{ number_format($bookItem->book->rental_price, 0, ',', '.') }}</span></p>
        </div>
      </div>

      <form action="{{ route('member.book-loans.store') }}" method="POST" class="mt-8 space-y-6">
        @csrf
